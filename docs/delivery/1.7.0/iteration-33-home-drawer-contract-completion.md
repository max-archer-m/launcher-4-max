...
Deliver the two remaining accepted-contract surfaces on Home and Drawer exactly as defined: the dismissible Home default-launcher prompt and the application-size block's two three-stop tapering sliders — with one author-device calibration pass that fixes the sliders' deliberately deferred visual values and records them back into the style-settings-panel contract.

## Product and version references

- Product-contract baseline: `da22d3d1c4c3e22105a8d6f96256c1e6ffe626d4` — the current mainline head, which contains the accepted default-launcher prompt contract (`7698498624d1d45efc940eb249a722423b593f68`) and the application-size tapering-slider contract.
- Applicable product documents:
  - [Home behavior](../../product/surfaces/home.md), default-launcher prompt section
  - [Home presentation](../../product/presentation/home.md), prompt presentation
...
- Style-settings-panel contract: calibration write-back through the authorized amendment path.
- Tests: focused local and instrumentation sources for the behaviors above.

## Dependencies and sequence

This iteration is independent of Iteration 32 in behavior and code areas, with shared-resource overlap only; by the version's serial default it follows Iteration 32. Its implementation line must contain the recorded baseline before production implementation proceeds. The calibration pass requires author-device participation and precedes the final acceptance of the slider work.
...
- The sliders are local presentation behavior.
...
- Calibration sessions with the author: slider candidates across the three tiers, with accepted values recorded.
- Glyph-shadow rendering on both Home and Drawer texts, including the Drawer background-opacity extremes from the background slider.
